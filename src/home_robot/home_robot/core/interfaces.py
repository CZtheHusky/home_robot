...
class Observations:
    ...
    base_pose: Optional[np.array] = None # (x, y, o)
    xyz: Optional[
        np.ndarray
    ...
    camera_angle: Optional[float] = None # in degrees, the camera view angle
    # Proprioreception
    ...
    # --------------------------------------------------------
    delta_pose: Optional[np.array] = None # dx, dy, do, do in radians!!!

    task_observations: Optional[Dict[str, Any]] = None
    # "instance_map": np.ndarray,  # (camera_height, camera_width) in [0, num_instances - 1], instance level
    # "instance_classes": np.ndarray,  # (num_instances,) in [0, num_sem_categories - 1], the class ids of the corresponding instances, one can revoer the class id of a instance in a frame with class_ids[instance_id]
    # "semantic_frame": np.ndarray,  # (camera_height, camera_width, 3) semantic segmentation frame
