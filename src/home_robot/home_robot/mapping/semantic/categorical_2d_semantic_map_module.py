...
class Categorical2DSemanticMapModule(nn.Module):
    ...
    def __init__(
        ...
        print("Camera parameter:", self.screen_h, self.screen_w, hfov)
        # breakpoint()
        ...
        print("shift loc:", self.shift_loc)
        ...
        self.num_envs = 1
        self.time_step = 0
        self.save_path = f"datadump/semantci_maps/"
        os.makedirs(self.save_path, exist_ok=True)
        # self.rgb_path = os.path.join(self.save_path, "rgb")
        self.pcd_path = os.path.join(self.save_path, "pcd")
        # self.depth_path = os.path.join(self.save_path, "depth")
        # self.seg_path = os.path.join(self.save_path, "seg")
        # self.obstacle_path = os.path.join(self.save_path, "obstacle")
        # self.close_path = os.path.join(self.save_path, "close")
        # self.explored_path = os.path.join(self.save_path, "explored")
        # os.makedirs(self.rgb_path, exist_ok=True)
        os.makedirs(self.pcd_path, exist_ok=True)
        # os.makedirs(self.depth_path, exist_ok=True)
        # os.makedirs(self.seg_path, exist_ok=True)
        # os.makedirs(self.obstacle_path, exist_ok=True)
        # os.makedirs(self.close_path, exist_ok=True)
        # os.makedirs(self.explored_path, exist_ok=True)
    ...
    def _update_local_map_and_pose(  # noqa: C901
        ...
        if camera_pose is not None:
            ...
            if camera_angle is None:
                tilt = np.zeros(batch_size)
            else:
                tilt = (- camera_angle).cpu().numpy()
            agent_height = self.agent_height

        yaw = torch.tensor(yaw)
        depth = obs[:, 3, :, :].float()
        depth[depth > self.max_depth] = 0
        # convert the depth to pointcloud based on the camera matrix
        point_cloud_t = du.get_point_cloud_from_z_t(
            depth, self.camera_matrix, device, scale=self.du_scale
        )

        if self.debug_mode:
            from home_robot.utils.point_cloud import show_point_cloud, numpy_to_pcd
            ...
            # tmp_pcd = show_point_cloud(
            #     (xyz / 100.0).cpu().numpy(),
            #     (rgb / 255.0).cpu().numpy(),
            #     orig=np.zeros(3),
            #     save=os.path.join(self.pcd_path, f"03d_rgb_{self.time_step}_geo.jpg"),
            # )
            tmp_pcd = numpy_to_pcd((xyz / 100.0).cpu().numpy(), (rgb / 255.0).cpu().numpy())
            o3d.io.write_point_cloud(os.path.join(self.pcd_path, f"03d_rgb_{self.time_step}.pcd"), tmp_pcd)
        # convert the point cloud to base coordinates with camera height and the view angel
        # point_cloud_base_coords = du.transform_camera_view_t(
        #     point_cloud_t, agent_height, torch.rad2deg(tilt).cpu().numpy(), device
        # )
        point_cloud_base_coords = du.transform_camera_view_t(
            point_cloud_t, agent_height, np.array(tilt), device
        )
        ...
        if self.debug_mode:
            ...
            # base_tmp_pcd = show_point_cloud(
            #     (xyz / 100.0).cpu().numpy(),
            #     (rgb / 255.0).cpu().numpy(),
            #     orig=np.zeros(3),
            #     save=os.path.join(self.pcd_path, f"03d_base_{self.time_step}_geo.jpg"),
            # )
            base_tmp_pcd = numpy_to_pcd((xyz / 100.0).cpu().numpy(), (rgb / 255.0).cpu().numpy())
            o3d.io.write_point_cloud(os.path.join(self.pcd_path, f"03d_base_{self.time_step}.pcd"), base_tmp_pcd)
        # convert the point cloud to map coordinates
        ...
        if self.debug_mode:
            ...
            # map_tmp_pcd = show_point_cloud(
            #     (xyz / 100.0).cpu().numpy(),
            #     (rgb / 255.0).cpu().numpy(),
            #     orig=np.zeros(3),
            #     save=os.path.join(self.pcd_path, f"03d_map_{self.time_step}_geo.jpg"),
            # )
            map_tmp_pcd = numpy_to_pcd((xyz / 100.0).cpu().numpy(), (rgb / 255.0).cpu().numpy())
            o3d.io.write_point_cloud(os.path.join(self.pcd_path, f"03d_map_{self.time_step}.pcd"), map_tmp_pcd)
        ...
        current_pose = pu.get_new_pose_torch(prev_pose[0].clone(), pose_delta[0])
        current_pose = current_pose.to(prev_pose.device)
        # print(f"Pose in map:\nprev_pose: {prev_pose[0]}\ncurrent_pose: {current_pose[0]}\npose_delta: {pose_delta[0]}")
        if self.record_instance_ids:
        ...
        if self.dilate_obstacles:
            for i in range(fp_map_pred.shape[0]):
                ...
                # median_filtered = cv2.medianBlur(env_map_eroded, self.dilate_size)
                env_map_dilated = cv2.dilate(
                    env_map_eroded, self.dilate_kernel, self.dilate_iter * 2
                )
                ...
                fp_map_pred[i, 0] = torch.tensor(env_map_dilated)
        ...
        if debug_maps:
            ...
            plt.subplot(233)
            plt.axis("off")
            plt.title("been close and explored")
            plt.imshow(been_close * explored)
            plt.subplot(334)
            plt.axis("off")
            plt.title("obstacles")
            plt.imshow(obstacles)
            x, y = curr_loc[0].cpu().numpy()
            ...
            plt.subplot(336)
            plt.axis("off")
            plt.title("close and obstacle")
            plt.imshow(been_close * obstacles)
            plt.subplot(337)
            plt.axis("off")
            # breakpoint()
            rgb = obs[0, :3, :: self.du_scale, :: self.du_scale].permute(1, 2, 0) / 255
            plt.title("rgb")
            plt.imshow(rgb.cpu().numpy())
            plt.subplot(338)
            plt.title("depth")
            plt.imshow(depth[0].cpu().numpy()* 255 / 150)
            plt.axis("off")
            plt.subplot(339)
            ...
            plt.axis("off")
            plt.show()
            plt.savefig(os.path.join(self.save_path, f"map_{step_cnt}.png"))
            plt.close()
