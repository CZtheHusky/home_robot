...
class InstanceMemory:
    ...
    def update_instance_id(
        ...
        if global_instance is None:
            ...
            global_instance.name = self.category_id_to_category_name[instance_view.category_id]
    ...
    def process_instances_for_env(
        ...
        """
        Process each instance and get its category id, point clound and cropped bounding box image
        """
        ...
        for instance_id in instance_ids:
            ...
            category_name = self.category_id_to_category_name[category_id]
            ...
            # import pdb; pdb.set_trace()
            if self.debug_visualize:
                # print("save instance view, id:", instance_id)
                os.makedirs(f"{self.save_dir}/all", exist_ok=True)
                cv2.imwrite(
                    f"{self.save_dir}/all/{self.timesteps[env_id] + 1}_{self.category_id_to_category_name[category_id]}_{instance_id.item()}.png",
                    cropped_image[:,:,::-1],
                )
    ...
    def process_instances(
        ...
        """To process instances in different environments, call process_instances_for_env() for each environment separately
        Args:
            semantic_channels (torch.Tensor): semantic image, from obs.semantic
            instance_channels (torch.Tensor): instance image, from obs.task_observations["instance_map"]
            point_cloud (torch.Tensor): point cloud
            pose (torch.Tensor): current position
