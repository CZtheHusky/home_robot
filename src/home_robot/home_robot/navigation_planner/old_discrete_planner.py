...
class DiscretePlanner:
    ...
    def __init__(
        ...
        continuous_angle_tolerance: float = 45,
    ):
        """
        Arguments:
    ...
    def plan(
        ...
        if debug:
            ...
            print("-----------------")
        stop = True if self.min_goal_distance_cm > distance_to_goal_cm else stop
        ...
        return action, closest_goal_map, short_term_goal, dilated_obstacles, replan, stop, planner_visual
    ...
    def get_action(
        ...
        if not (found_goal and stop):
            ...
            if self.discrete_actions:
                ...
                #     action = DiscreteNavigationAction.TURN_LEFT
                if relative_angle_to_closest_goal > self.turn_angle / 2.0:
                    action = DiscreteNavigationAction.TURN_RIGHT
                elif relative_angle_to_closest_goal < - self.turn_angle / 2.0:
                    action = DiscreteNavigationAction.TURN_LEFT 
                else:
    ...
    def _get_short_term_goal(
        ...
        if visualize:
            ...
            if navigable_goal_map is not None:
                _navigable_goal_map = navigable_goal_map.copy()
                _navigable_goal_map[int(stg_x), int(stg_y)] = 1
            else:
                _navigable_goal_map = None
            planner_visual = {
                "navigable_goal_map": _navigable_goal_map, 
                "goal_map": goal_map,
                "fmm_dist": planner.fmm_dist,
                "traversible": planner.traversible,
                "stg": short_term_goal,
                "pos": start,
    ...
    def _check_collision(self):
        ...
        cur_r, cur_c = int(x2 * 100 / self.map_resolution), int(
            y2 * 100 / self.map_resolution
        )
        # buf = 4
        # length = 2
        # delta_pose = pu.get_rel_pose_change(self.last_pose, self.curr_pose)
        theta = - np.deg2rad(t1)
        dx = np.cos(theta)
        dy = np.sin(theta)
        width = 6
        ...
        # print("collision position:", cur_r, cur_c)
        if dist < self.collision_threshold:
            for i in range(width):
                for j in range(col):
                    wx = x1 - 0.05 * (i - 3) - 0.25 * dx
                    wy = y1 - 0.05 * (j - 3) - 0.25 * dy
                    r, c = wy, wx
                    r, c = int(r * 100 / self.map_resolution), int(
                        c * 100 / self.map_resolution
                    )
                    [c, r] = pu.threshold_poses([c, r], self.collision_map.shape)
                    # print("adding to collision map:", r, c)
