...
def get_rel_pose_change(pos2, pos1):
    '''
    pos1: x, y, o, o in degree
    pos2: x, y, o, o in degree
    '''
    x1, y1, o1 = pos1
    x2, y2, o2 = pos2
    theta = np.arctan2(y2 - y1, x2 - x1) - np.deg2rad(o1)
...
def get_new_pose_torch(pose, rel_pose_change):
    x, y, o = pose
    dx, dy, do = rel_pose_change
    global_dx = dx * torch.sin(torch.deg2rad(o)) + dy * torch.cos(torch.deg2rad(o))
    global_dy = dx * torch.cos(torch.deg2rad(o)) - dy * torch.sin(torch.deg2rad(o))
    x += global_dy
    y += global_dx
    o += torch.rad2deg(do)
    if o > 180.0:
        o -= 360.0
    return torch.tensor([[x, y, o]])

def get_new_pose_batch(pose, rel_pose_change):
    '''
    pose: x, y, o, o in degrees    
    rel_pose_change: dx, dy, do, do in degrees
    '''
    const = 57.29577951308232   # 180/pi
